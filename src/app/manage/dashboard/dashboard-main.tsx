...
const initFromDate = startOfDay(new Date(new Date().setMonth(new Date().getMonth() - 1)));
			...
			<div className="flex flex-wrap gap-2">
				<div className="flex items-center">
					...
					<Input
						type="datetime-local"
						placeholder="Từ ngày"
						className="text-sm" // Định dạng giá trị ngày thành dạng datetime-local
				...
				<div className="flex items-center">
					...
					<Input
						type="datetime-local"
						placeholder="Đến ngày" // Định dạng giá trị ngày thành dạng datetime-local
						value={format(initToDate, "yyyy-MM-dd HH:mm").replace(" ", "T")}
						// Cập nhật state fromDate khi người dùng thay đổi
						onChange={(event) => setFromDate(new Date(event.target.value))}
					/>
				</div>
