...
export default function AddTable() {
  ...
  const onSubmit = async (values: CreateTableBodyType) => {
    ...
    try {
      ...
      reset();
      setOpen(false); // sau khi thêm thành công thì đóng dialog
    } catch (error) {
      handleErrorApi({
        error,
  ...
  return (
    <Dialog
      onOpenChange={(value) => { // Hàm callback này sẽ được gọi khi dialog mở hoặc đóng
        ...
        setOpen(value); // Cập nhật state open với giá trị mới
      }}
      open={open} // Điều khiển trạng thái mở/đóng của dialog
    >
